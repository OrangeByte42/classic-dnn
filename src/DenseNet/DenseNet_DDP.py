...
from utils.ddp.densenet import DenseNet, init_weights
...
if __name__ == '__main__':
    ...
    densenet_121_model: nn.Module = DenseNet(in_channels=1, num_init_features=64, num_classes=10,
                                            block_config=(6, 12, 24, 16), growth_rate=32, bn_size=4, dropout=0.0)
    densenet_121_model.apply(init_weights)  # Initialize weights
    dist.barrier()  # Ensure all processes have initialized the model before proceeding

    # Setup training hyper-parameters
    num_epochs: int = 50
    lr: float = 5e-2
    net: Any = DDP(densenet_121_model.to(torch.device(rank)), device_ids=[rank], output_device=rank)  # Move model to the appropriate device
