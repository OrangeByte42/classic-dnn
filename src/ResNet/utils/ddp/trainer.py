...
def _train_epoch(net: Any, train_iter: Any, loss: Any, trainer: Any, rank: int) -> Tuple[float, float]:
...
def train(net: Any, train_iter: Any, test_iter: Any,
    ...
    for epoch in range(num_epochs):
        ...
        train_metrics: Tuple[float, float] = _train_epoch(net, train_iter, loss, trainer, rank)
        ...
        if rank == 0:
            checkpoint_path: str = os.path.join(checkpoint_dir, f'resnet50_epoch_{epoch + 1}_checkpoints.pth')
