...
from utils.ddp.resnet import resnet18, resnet50, init_weights
...
if __name__ == '__main__':
    ...
    resnet_model: nn.Sequential = resnet50(in_channels=1)
    resnet_model.apply(init_weights)  # Initialize weights
    dist.barrier()  # Ensure all processes have initialized the model before proceeding

    # Setup training hyper-parameters
    num_epochs: int = 50
    lr: float = 1e-2
    net: Any = DDP(resnet_model.to(torch.device(rank)), device_ids=[rank], output_device=rank)  # Move model to the appropriate device
