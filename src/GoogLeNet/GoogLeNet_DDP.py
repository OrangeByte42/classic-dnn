...
from utils.ddp.googlenet import googlenet, init_weights
...
if __name__ == '__main__':
    ...
    googlenet_model: nn.Sequential = googlenet(in_channels=1)
    googlenet_model.apply(init_weights)  # Initialize weights
    dist.barrier()  # Ensure all processes have initialized the model before proceeding

    # Setup training hyper-parameters
    num_epochs: int = 100
    lr: float = 5e-2
    net: Any = DDP(googlenet_model.to(torch.device(rank)), device_ids=[rank], output_device=rank)  # Move model to the appropriate device
