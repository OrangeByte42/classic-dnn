...
from utils.ddp.vgg import vgg, init_weights
...
if __name__ == '__main__':
    ...
    # Instantiate VGG-19 model
    conv_arch: Tuple[Tuple[int, int]] = ((2, 64), (2, 128), (4, 256), (4, 512), (4, 512))
    vgg_19: nn.Sequential = vgg(in_channels=1, conv_arch=conv_arch)
    vgg_19.apply(init_weights)  # Initialize weights
    dist.barrier()  # Ensure all processes have initialized the model before proceeding

    # Setup training hyper-parameters
    num_epochs: int = 100
    lr: float = 1e-2
    net: Any = DDP(vgg_19.to(torch.device(rank)), device_ids=[rank], output_device=rank)  # Move model to the appropriate device
